      ...
      <main className="mx-auto max-w-3xl px-6 py-12">
        ...
        <section className="grid gap-4 md:grid-cols-2">
          <div className="rounded-xl border border-hairline bg-surface p-6">
            <p className="font-display text-[56px] leading-none tracking-tight text-ink">
                ...
                <li key={d.k} className="flex items-center gap-3">
                  <span
                    className={`w-12 shrink-0 text-right font-display text-2xl tabular-nums ${
                      d.forte ? "text-ink" : "text-ink-soft"
                    }`}
                  >
                    {p < 1 ? "<1" : p}%
                  </span>
                  ...
                  <span className="w-64 shrink-0 text-sm text-ink-soft">{d.rotulo}</span>
                </li>
              );
            })}
          </ul>
              ...
              <tbody className="font-data">
                {verticais.map((v) => (
                  <tr key={v.id} className="border-b border-hairline last:border-0">
                    <td className="px-4 py-3 text-ink">{v.nome}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{fmt(v.total)}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{fmt(v.independentes)}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink">{fmt(v.quente)}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink-soft">~{v.deals_ano}</td>
                  </tr>
                ))}
              </tbody>
            </table>

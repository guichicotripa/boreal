    ...
    <div className="min-h-screen bg-canvas text-ink">
      <main className="mx-auto max-w-3xl px-6 py-12">
        <header className="mb-10 flex items-start justify-between">
          ...
          <Link href="/" className="group flex items-center gap-2 rounded-sm font-data text-[11px] uppercase tracking-wider text-ink transition-opacity hover:opacity-70 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ink/50">
        ...
        <section className="rounded-xl border border-hairline bg-surface p-8">
          ...
          <p className="flow-root text-[19px] leading-normal text-ink-soft">
            <span className="float-left mr-5 mt-1 font-display text-[72px] leading-[0.85] tracking-tight text-ink">
              ...
              <tbody>
                {(hindcast.deals as Deal[]).slice(0, 10).map((d) => (
                  <tr key={d.nome} className="border-b border-hairline last:border-0">
                    <td className="px-4 py-3 text-ink">{d.nome}</td>
                    <td className="px-4 py-3 text-ink-soft">{d.municipio}</td>
                    <td className="px-4 py-3 text-right font-data tabular-nums text-ink-soft">
                      {d.ano_deal ?? "—"}
                    </td>
                    <td className="px-4 py-3 text-right font-data tabular-nums text-ink">
        ...
        <section className="mt-10">
          ...
          <ol className="mt-4 space-y-4">
            <li className="flex gap-4">
              <span className="font-display text-2xl text-ink">1</span>
              <p className="text-ink-soft">
                <strong className="text-ink">Ground truth de graça.</strong> Comparamos dois
            ...
            <li className="flex gap-4">
              ...
              <p className="text-ink-soft">
                <strong className="text-ink">Zero leakage.</strong> O score é calculado só com os
            ...
            <li className="flex gap-4">
              ...
              <p className="text-ink-soft">
                <strong className="text-ink">Medimos o recall.</strong> Das empresas que de fato
              ...
              <tbody className="font-data">
                {verticais.map((v) => (
                  <tr key={v.vertical} className="border-b border-hairline last:border-0">
                    ...
                    <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{fmt(v.universo)}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{v.n_aquisicoes}</td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink">
                      {v.recall_top10}%
                    </td>
                    <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{v.lift_top10}×</td>
                  </tr>
                ))}
              </tbody>
            </table>
                ...
                {(lift.features as Feature[])
                  ...
                  .map((f) => (
                    <tr key={f.nome} className="border-b border-hairline">
                      ...
                      <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{f.universo_pct}%</td>
                      <td className="px-4 py-3 text-right tabular-nums text-ink-soft">{f.adquiridas_pct}%</td>
                      <td className="px-4 py-3 text-right tabular-nums text-ink">
        ...
        <div className="mt-10 flex justify-center">
          <Link
            href="/"
            className="group flex items-center gap-2 rounded-sm font-data text-[11px] uppercase tracking-wider text-ink transition-opacity hover:opacity-70 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-ink/50"
          >

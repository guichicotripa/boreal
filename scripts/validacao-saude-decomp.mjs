// Decompõe as aquisições de SAÚDE por perfil da empresa-alvo no corte, pra testar a hipótese
// "saúde = consolidação, não sucessão". Mede recall@top10% do score v0.1 em cada bucket.
// Bucket pelo perfil sucessório do alvo: sócio idoso (faixa>=7) e/ou empresa antiga (25+ anos).
// node --env-file=.env.local scripts/validacao-saude-decomp.mjs
...
console.log(`Aquisições de SAÚDE decompostas (N=${tot}):\n`);
for (const r of rows) {

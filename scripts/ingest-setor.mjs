/**
 * Ingest sector-aware: BigQuery br_me_cnpj → Supabase, para UM setor do registry.
 * Aumenta cobertura sem tocar nos setores já ingeridos (upsert idempotente por cnpj).
 *   node --env-file=.env.local scripts/ingest-setor.mjs saude
 *   node --env-file=.env.local scripts/ingest-setor.mjs educacao
 *
 * Mesma lógica do ingest-empresas (enrichment N0 via JOIN), mas o filtro CNAE vem do
 * setor (src/lib/setores.json). Top 2000 por risco sucessório (faixa etária máx DESC).
 */
...
const setorId = (process.argv[2] || "").trim();
if (!setorId) { console.error("uso: node ingest-setor.mjs <setorId>  (ex: saude, educacao)"); process.exit(1); }
...
console.log(`Setor "${setor.nome}" (CNAE ${setor.cnaes.join("/")}) · SP · top ${LIMIT} por risco sucessório\n`);
...
const empresasSql = `
  ...
  WHERE e.data = '${SNAPSHOT}' AND ${cnaeFiltro} AND e.sigla_uf = 'SP'
